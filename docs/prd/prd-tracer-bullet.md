...
- **日期**：2026-08-14（进度更新：2026-08-15）
- **状态**：✅ 贯通线已达成（跑在动态插件上）；进入「质量与固化」阶段
...
## 0. 进度追踪（2026-08-15 更新）

### 0.1 贯通线验收（§2 成功标准）：全部达成

| 验收项 | 状态 | 证据 |
|---|---|---|
| 自然语言建书 → 书目录+状态文件 | ✅ | `novel_create_book`（slug+hash6 bookId，`novels/book-99b16c/`） |
| 「写一章」一次调用完成写作+检测+落盘 | ✅ | `novel_write_chapter`（实测两章：001/002） |
| 评分来自真实检测器 | ✅ | 12 维 `detectAI`，评分随章节元数据落盘（ch1=100, ch2=100） |
| `chapters/001.md` + `story/state/state.json` 一致 | ✅ | 校验后写入（validateState 拒绝坏数据），110 测试锁行为 |
| 章节面板列出+点开读正文 | ✅ | 右下角「小说」面板：画布分栏（节点栏+正文）+ 结构树两视图，`host.call` RPC |

### 0.2 FR 完成度

| FR | 状态 | 备注 |
|---|---|---|
| FR-1 建书工具 | ✅ | 含重名检测、bookId 安全校验 |
| FR-2 写章工具 | ✅ | `words`/`context` 参数齐 |
| FR-3 六阶段流水线 | ✅ 超额 | 实际为 **Planner备忘录 → 分层Writer → 检测 → 外科修订 → Observer结算 → 连续性审计**（超出 PRD 的单条链，见 0.4） |
| FR-4 detect/rewrite | ✅ | detect=12维确定性纯函数；rewrite=外科手术式（只修命中句，保留其余原文） |
| FR-5 写前校验 | ✅ | `validateState` 拒绝写入 |
| FR-6 原子读-改-校验-写 | ✅ | |
| FR-7 list/read 工具 | ✅ | 4 工具 + 6 RPC（含 debug_info、get_structure、list_outline） |
| FR-8 常驻面板 | ✅ | `shell.overlay` FAB+面板 |
| FR-9 结构化结果 | ✅ | 结果串含章节号/字数/评分/路径/修订标记/审计警告 |

### 0.3 用户故事完成度：US-001/002/003 全部达成

### 0.4 超出 PRD 的新能力（inkos 方法论移植，本轮新增）

1. **Planner 备忘录制度**（学 inkos `planner.ts`/`planner-prompts.ts`）：每章先由「总编」LLM 产出十节结构化备忘录（本章目标≤50字/当前任务/读者在等什么/场景规划含字数预算与感官焦点/该兑现的暂不掀的/章尾必须改变/伏笔动作账/不要做/风格强调），落盘 `story/runtime/chapter-NNN.memo.md`；写手只按备忘录扩写。
2. **Writer 分层系统提示词**（学 inkos `writer-prompts.ts`，替代口号式清单）：写作铁律（带 before/after 示例）→ 文笔执行（明喻节制≤1/场、高潮必须演出）→ 创作宪法14条（散文体，禁引用）→ 代入感六支柱 → **黄金三章纪律**（第≤3章条件加载）→ 输出约定。**禁用词清单已从提示词移除**（负向锚定问题），检测全部后置到落盘前的确定性检测层。
3. **文风指纹**（学 inkos style_fingerprint）：`story/style_fingerprint.md` 存在则注入「模仿目标」段——从本书定稿正文提炼的人味特征（细节等级/比喻纪律/农民计时法/收尾方式）。
4. **逐字事实侧栏**（学 inkos protected context）：`story/verbatim.md` 声明道具原文（布片六字/口诀三句/专名/硬设定），Writer 引用必须照抄——修复了第2章暴露的「道具文本漂移」硬伤（口诀被意译）。
5. **前文摘要注入**：Writer 现在能看到前章摘要（连续性事实）。
6. **伏笔生命周期**：open/progressing/deferred/resolved 状态机、半衰期、stale/blocked 诊断、`current_focus.md`/`pending_hooks.md`/`chapter_summaries.md` 人类可读投影。
7. **连续性审计**：新角色/位置突变/伏笔冲突落 `runtime/chapter-NNN.audit.md`。
8. **写章进度卡**：`presentCall` 调用前说明（2-3次模型调用、预计时长）；`exec.signal` 全链路中止支持（用户中止立即停止，不再浪费 token）。
9. **Host 引导加载器**：部署的宿主半仅 ~30 行探针，激活时从磁盘读 `plugins/host-novel.js` 源码执行——改提示词只需改磁盘文件+重启插件，无需重新 cordis_define 整段代码。
10. **110→120 个 parity 测试**：`src/` 纯函数 ↔ host 插件内联副本 ↔ 测试三方锁行为，防漂移。

### 0.4b Skill 方法论移植（2026-08-15 第二轮，pkg-2）

对工作区 `skills/`（anti-ai-flavor / longform-writing / novel-qa / novel-style-reference / novel-outline-researcher / distill-novels）逐份研讨后，取其精华去其糟粕：

**移植的精华（检测器 12→18 维 + 流水线 4 处）：**

| 来源 | 移植内容 | 落点 |
|---|---|---|
| anti-ai §一 | 「不是X是Y」≤2次/章免费（实战 663→31 的最大AI指纹） | `countBushiXY` dim13 |
| anti-ai §一 | 全角破折号 ≤20/章 | `countEmDash` dim14 |
| anti-ai §一 | 正文元话语 0 容忍（前文所述/后文再表/卷X） | `detectMetaDiscourse` dim15 |
| anti-ai §六 | 结尾禁句（他不知道的是/这只是开始/一切都变了…只查尾120字） | `detectBannedEndings` dim16 |
| anti-ai §六 | 开场前300字物理事件 + 前500字对话（≥600字才罚） | `checkOpening` dim17 |
| anti-ai §一/§六 | 四字格连排（整句全四字单元才命中，普通分句不误伤） | `detectIdiomStack` dim18 |
| anti-ai §一 | 章内重复表达（≥5字n-gram 出现≥3次） | `repeatedNgrams` dim19 |
| anti-ai §一/§九 | 跨章签名短语：前文≥3次的表达本章禁用，三章一轮扫描 | `crossChapterRepeats`（index%3==0 触发） |
| longform §结构 | 单章结构预算：开场10%直进事件/发展60%冲突升级/高潮20%/结尾10%留钩 | Planner 提示词 |
| longform §钩子 | 钩子十三式 + 上章已用类型本章禁用（轮换） | Planner 备忘录新增「章尾钩子设计」节 |
| longform §节奏 | 情绪债务节奏：小爽点每2-3章；担忧型≤5章回收/委屈型≤10章/期待型≤30章 | Planner 提示词 |
| longform §水章 | 水章诊断6条件（命中≥3即报，下一章必须推进） | Observer 提示词 + 警告流 |
| anti-ai §八 | 人味正面清单：模糊词是诚实/动作裹判断/无用细节/对话损耗（说漏说错被打断说一半） | Writer 提示词正向段 |
| anti-ai §九 | 反AI三问（能拍出来吗/真人会说吗/去掉影响吗） | 外科修订提示词 |
| novel-qa | 评分语义澄清：UI 标签 AI100→净100（防"100%AI味"误读） | 客户端 |

**清除的糟粕（我们之前抄错的）：**

1. **HEDGE_WORDS 惩罚似乎/可能/或许/大概**——skill 明文：「允许并鼓励模糊词：大概、也许、我隐约记得——这不是偷懒，是诚实」；novel-qa：「软性内心活动不得标记」。**我们在惩罚人味**。已改为只罚学术腔（某种程度上/众所周知/毋庸置疑）。
2. **rewriteRules「减少套话（似乎/可能/或许）」**——同上，已删并加反向说明。
3. **「不是XX是XX」每次出现都扣4分**——skill 实战数据是允许2次免费，超量才是指纹。已改为专管函数+免费额度。
4. **（保留的弃用）skill §用法说「把硬约束原样注入写手提示词」**——刻意不抄。inkos 移植轮已证明负向清单注入会造成反向锚定（用户反馈「味道大」的直接原因）。skill 的检测规则全部后置到确定性检测层，提示词只给正向示范。

**novel-outline-researcher / distill-novels**：交互式调研流程与蒸馏工具属人工协同场景，自动流水线不直接移植；其「先读前文再规划」原则已通过前文摘要注入体现。
...
| # | 项 | 现状 | 优先级 |
|---|---|---|---|
| DEBT-1 | **第1章违反黄金三章纪律**：前 300 字无反差收尾钩、开篇是慢热日常（写于纪律注入之前） | 文本已落盘未按新纪律重写 | 高（影响成品书开篇留存） |
| DEBT-2 | **verbatim 漂移守卫未自动化**：逐字事实靠提示词约束，落盘前无确定性校验（第2章漂移是人工抓到的） | 需在 audit 阶段加「verbatim 项 grep 校验」 | 高（同类型事故会复发） |
| DEBT-3 | E8 字数治理 `lengthWarning` 未实现（words 是目标，无区间推导与标注） | — | 中 |
| DEBT-4 | E11 书级并发锁 `BOOK_BUSY` 未实现 | 单会话使用无感知 | 中 |
| DEBT-5 | E14 章级快照/回滚未实现 | state.json 可 git 兜底 | 中 |
| DEBT-6 | E6 中断部分保留未实现（中止即丢，不落半章） | — | 低 |
| DEBT-7 | **预设固化未做**（PRD §7.5/§8.5）：仍是动态插件，重启进程即丢（本会话已重建过一次） | 需固化进 preset 目录 | 高（交付物稳定性） |
| DEBT-8 | 技能包未接线：`skills/anti-ai-flavor` 等 SKILL.md 在工作区但预设未引用 | — | 中（随 DEBT-7 一起做） |
| DEBT-9 | E2 多书歧义消解未做（工具要求显式 bookId，靠 agent 侧判断） | — | 低 |
| DEBT-10 | 检测器假阳性：粗体标记触发「连续3句相同开头」（已人工绕过，未修检测器） | — | 低（列表检测可忽略引号/星号前缀） |
| DEBT-11 | 情绪债务未入 hook 状态模型：担忧≤5章/委屈≤10章/期待≤30章 目前只在 Planner 提示词里口头约束，未做成 hook.emotion 字段 + 诊断 | 需扩 state-schema + hook-lifecycle + 测试 | 中（跨5章后价值显现） |
| DEBT-12 | 对话占比目标 30% 只计量不惩罚（风格留白判断给了 Planner 引导）——若后续要硬卡需按题材分级阈值 | dialogueRatio 已计量进 breakdown | 低 |

### 0.6 下一步建议顺序

1. **DEBT-2**（verbatim 守卫自动化，小改动高收益）→ 2. **DEBT-1**（按黄金三章纪律重写第1章）→ 3. **DEBT-7+8**（固化预设+技能接线，交付物不再随进程蒸发）→ 4. DEBT-3/4/5（治理项）→ 5. 连写多章验证伏笔账本跨章结算 → 6. 短篇参数化（PRD 非目标解禁）。
